...
func ReadQueryString(qs url.Values, key string, v *validator.Validator) string {
	queryValue := qs.Get(key)

	v.Check(queryValue != "", key, fmt.Sprintf("Key %s cannot be empty", key))
...
func ReadQueryInt(qs url.Values, key string, v *validator.Validator) int {
	...
	intQueryValue, err := strconv.Atoi(queryValue)

	if err != nil {
		v.AddError(key, fmt.Sprintf("Key %s key must be an int", key))
	}

	return intQueryValue
}

func ReadQueryCSV(qs url.Values, key string, v *validator.Validator) []string {
	queryValue := qs.Get(key)

	v.Check(queryValue != "", key, fmt.Sprintf("Key %s cannot be empty", key))

	return strings.Split(queryValue, ",")
}
